...
socket.on('newLink', async ({ link, wallet }) => {
  ...
  if (userScore >= 0.5) {
    ...
    saveChatLog({ fromUser: nickname, message: link });
    console.log(`💾 chatLogsDB 저장: ${nickname} -> ${link}`);
  } else {
...
socket.on('linkClicked', async ({ fromUser, toUser, link }) => {
  ...
  if (score >= 0.5) {
    ...
    try {
      let wb, ws, arr;
      try {
        wb = xlsx.readFile(CLICK_DB_PATH); // 여기서 dhodksehoDB.xlsx 경로 사용
        ws = wb.Sheets[wb.SheetNames[0]];
        arr = xlsx.utils.sheet_to_json(ws, { header: 1 });
      } catch {
        // 파일이 없으면 새로 만들기
        wb = xlsx.utils.book_new();
        arr = [['fromUser', 'toUser']]; // 헤더 추가
        ws = xlsx.utils.aoa_to_sheet(arr);
        xlsx.utils.book_append_sheet(wb, ws, 'Sheet1');
      }

      // 새로운 행 추가 (fromUser -> toUser)
